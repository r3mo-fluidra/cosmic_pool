...
# ==========================================
def render_details(sections: list[tuple[str, str]]) -> None:
    """
    The synthesizer's `details`, one expander per section.

    Until this existed, tier 2 was written, budgeted, validated and thrown
    away: the app renders `tier1_markdown()`, which omits `details` entirely.
    That made `normalize_actions` DELETE rather than relocate — the fifth
    action of a specialist that returned six went to a section nobody painted,
    and the "never deleted, only relocated" guarantee in response_validator.py
    held for the object and not for the screen.

    Called from the history loop, like `render_debug` and the feedback row and
    for the same reason: the turn ends in `st.rerun()`, so anything rendered
    during it is discarded a moment later.
    """
    for label, body in sections or []:
        if not (label and body and body.strip()):
            continue
        ...
        with screen_scroll:
            ...
            for msg in st.session_state.messages:
                with st.chat_message(msg["role"]):
                    ...
                    # Tier 2, under the answer and inside the card: the
                    # reasoning belongs to the answer, unlike the action row.
...
def run_turn(
    ...
    status_slot=None,
    answer_slot=None,
) -> tuple[str, bool, list[tuple[str, str]], list[str], list[tuple[str, str]]]:
    """
    ...
    pipeline-debug lines (see `render_debug`), the suggestion chips, and the
    synthesizer's `details` sections as `(label, body)` pairs (see
    `render_details`).
    ...
    for mode, payload in graph.stream(
        ...
        if mode == "messages":
            ...
            # Los sub-agentes. Su contenido es el JSON del contrato interno y
            # no se muestra, pero sus tool calls sí dicen algo honesto sobre
            # qué está pasando — y son los 15-19s en los que el stack estaba
            # congelado en la línea del agente, porque `step_finished` solo
            # llega cuando el sub-agente terminó ENTERO.
            #
            # `detail` y no `advance`: una tool call ocurre DENTRO de un step,
            # no al lado. Con un presupuesto de seis llamadas, `advance`
            # apilaría siete filas en una pantalla de móvil.
            #
            # `name` solo viene en el primer chunk de cada tool call; los
            # siguientes traen los argumentos con name=None.
            called = False
            for call in getattr(chunk, "tool_call_chunks", None) or []:
                name = call.get("name")
                if name:
                    progress.detail(tool_line(progress.language, name))
                    called = True

            # Primer chunk de texto sin tool call: el especialista dejó de
            # buscar y está redactando. Es el bloque más largo del turno
            # (9-14s medidos) y hasta ahora no tenía nombre en pantalla.
            if not called and chunk_text(chunk):
                progress.detail(tool_line(progress.language, "_writing"))
            continue
        ...
        if "planner" in event:
            ...
            # Tier 2. `response` is the SynthesizerOutput the node writes into
            # state; `messages[-1].content` is only its tier1_markdown(), so
            # without this the collapsible sections never leave the graph.
            synth_payload = event["synthesizer"].get("response")
            if synth_payload is not None:
                details = [
                    (d.label, d.body)
                    for d in (getattr(synth_payload, "details", None) or [])
                ]
...
if prompt:
    ...
    if lf is not None:
        ...
        try:
            # Cinco valores, igual que la rama con Langfuse de arriba.
            # Desempaquetar de menos reventaba este camino en CADA turno con
    ...
    if final_response:
        ...
        #
        # Los expanders de tier 2 tampoco se pintan acá: como el panel de
        # debug y la fila de feedback, los renderiza el bucle de historial
        # después del rerun, que es donde sobreviven.
        #
        ...
        answer: ChatMessage = {
            ...
            # Tier 2, as (label, body) pairs. Kept on the message rather than
            # re-derived, because the SynthesizerOutput is gone by the time the
            # history loop runs.
            "details": details,
        }
